...
from keras.layers import MaxPooling2D, GlobalAveragePooling2D, Dropout, Conv2D
...
dropout = False			# Whether to use dropout or not
batch_norm = False		#    "    "   "  batch normalization " "
drop_prob = 0.5
n_examples = 1881		# Number of training examples
# Note: if n_examples > 1881 data augmentation is applied

# Task-specific model weights (topModel==> added layers)
model_fname = "CNN-n_examples=" + str(n_examples) +\
...
# Define the basic (and maybe naive) model (2 conv. layers + 2 FC)
...
model_checkpoint = ModelCheckpoint(model_fname, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
